...
class AlojamientoController extends Controller
{
  ...
  /**
     ...
     * @return Response
     */
  public function alojamientoAction(Request $request, MailerInterface $mailer, GlobalDirection $globalDirection, SeoData $seoData, SendEmails $sendEmails, $slug, $slugg, $sluggg, $menulocal): Response
  {
    $em = $this->getDoctrine()->getManager();

    $aaa0 = $this->alojamientosRepository->findAlojamiento0($sluggg);

    $localidad = $aaa0[0]['centroTuristico'];
    $provincia = $aaa0[0]['provincia'];

    $aaat12 = $this->alojamientosRepository->findAlojamientot12($localidad, $provincia);
    $filas = count($aaat12);
    $tipos = array();
    $i = 0;
    while ($i < $filas) {
    ...
    $ppp2 = $this->alojamientosRepository->findAlojamiento2($localidad, $provincia, $sluggg);
    $aaa3 = $this->alojamientosRepository->findAlojamiento3($localidad, $provincia);
    ...
    $local2aux = $this->alojamientosRepository->findLocalidades2aux($latitud1, $longitud1);
    ...
    while ($i < $filas1aux) {
      ...
      $ppp2aux = $this->alojamientosRepository->findAlojamiento2aux($localidad, $provincia);
      if (count($ppp2aux) > 0) {
        $arrayppp2aux = array_merge($arrayppp2aux, $ppp2aux);
      }
      $aaa3aux = $this->alojamientosRepository->findAlojamiento3($localidad, $provincia);
      $arrayaaa3aux = array_merge($arrayaaa3aux, $aaa3aux);

      $aaat12aux = $this->alojamientosRepository->findAlojamientot12($localidad, $provincia);
